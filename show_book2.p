...
v-isbn     = entry(1,session:parameter).

ASSIGN
v-mod     = OS-GETENV("cambia")
v-del     = OS-GETENV("borra").
v-mod     = entry(4,session:parameter).
v-del     = entry(5,session:parameter).
v-cant    = entry(6,session:parameter). 

/* {&OUT} "mod:" v-mod  " del:" v-del " cant:" v-cant. */
...
{&OUT} '        
    <center><a href="?p=show_cart.p&user=' v-user '&ord=' v-ord '">
